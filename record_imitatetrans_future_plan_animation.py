"""
Top-down animation of ImitateTrans_singlemap.py over a GRF map, showing the
...
same convention as record_cmaes_future_plan_animation.py and
record_realgreedy_future_plan_animation.py.

Mirrors ImitateTrans_singlemap.py's own __main__ loop (3D waypoint/dynamics
from ts=0, current_heading_velocity tracked every step since the model
conditions on it) rather than CMA-ES's 2D-only initial phase. Uses the live
default checkpoint (ImitateTrans_singlemap.imitate_checkpoint_path), not
overridden. Background is the GP posterior mean field.
"""
import os
...
os.environ.setdefault("EXECUTION_CHUNK", "40")  # match the diffusion planner's horizon for a fair comparison
...
import ImitateTrans_singlemap as sim
from gaussianprocesstraining import fov_lateral_radius, initialize_gp

SCRIPT_DIR = Path(__file__).resolve().parent
OUT_MP4 = SCRIPT_DIR / f"imitatetrans_future_plan_map{sim.selected_map}_{sim.MAPTYPE}.mp4"
TITLE = "ImitateTrans"
...
def main():
    ...
    print(f"Loading ImitateTrans checkpoint: {sim.imitate_checkpoint_path}")
    model = sim.load_imitate_model(checkpoint_path=sim.imitate_checkpoint_path)
    ...
    print("Running ImitateTrans flight over GRF map (real inference each replan)...")
    for ts in range(0, sim.timealloted):
        if ts <= 1:
            ...
            if ts == 2 or spline_idx >= sim.execution_chunk or spline_idx >= len(spline_path):
                ...
                dense_traj, sparse_controls = sim.sample_imitation_trajectory(
                    model,
