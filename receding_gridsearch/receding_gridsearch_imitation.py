...
import MLPimitationlearner as imitation
...
imitation_path = r"C:\Users\Aksha\OneDrive\Year 6\Thesis\scripts\Diffusion\checkpoints\mlp_imitation_traj_final.pth"

"""
Single-map evaluation copy of receding_gridsearch.py using the deterministic
imitation learner baseline instead of diffusion denoising. All default settings
remain aligned with receding_gridsearch.py, but this file runs one chosen map
and reports task completion at the end.
"""


def load_imitation_model(checkpoint_path=None):
    model = imitation.TrajPredictor().to(imitation.device)
    if checkpoint_path is not None:
        state_dict = torch.load(checkpoint_path, map_location=imitation.device)
...
@torch.no_grad()
def sample_imitation_trajectory(model, current_position, current_mean, current_var):
    current_position = torch.as_tensor(current_position, dtype=torch.float32, device=imitation.device).view(1, 2)
    current_position = current_position / imitation.SCALE_FACTOR
    ...
    mean_map = (current_mean - imitation.mean_center.to(imitation.device)) / imitation.mean_scale.to(imitation.device)
    log_var_map = torch.log1p(current_var)
    log_var_map = (log_var_map - imitation.log_var_center.to(imitation.device)) / imitation.log_var_scale.to(imitation.device)
    meanvar_map = torch.stack([mean_map, log_var_map], dim=0).unsqueeze(0)

    traj = model(meanvar_map, current_position)
    traj = (traj[0].detach().cpu().numpy() * imitation.SCALE_FACTOR)
    return traj
...
if __name__ == "__main__":
    ...
    for selected_map in range(initial_map, initial_map + mapcount):
        ...
        data = np.loadtxt(rf"{path}/map_{selected_map}_blob_grid_counts.csv", delimiter=",", skiprows=1)
        ...
        for ts in range(0, timealloted):
            if ts <= 10:
                ...
                if ts == 11:
                    current_mean = mu.reshape(51, 51)
                    current_var = np.diag(P).reshape(51, 51)
                    flight_plan = sample_imitation_trajectory(imitation_model, current_position=(cx, cy), current_mean=current_mean, current_var=current_var)
                    flight_plan = flight_plan.T.tolist()
                    # flight_plan = flight_plan[1:].tolist()
                

                print("Current imitation flight plan:", flight_plan)
                ...
                if len(flight_plan) <= action_horizon:
                    ...
                    flight_plan = flight_plan.T.tolist()
                    # flight_plan = flight_plan[1:].tolist()
                    print("Replanning imitation plan...")
        ...
        # print(f"Map {selected_map} true blob utility captured after {timealloted} timesteps: {metrics['gained_true_utility']:.4f}")
        # print(f"Map {selected_map} total true blob utility: {metrics['total_true_utility']:.4f}")
        # print(f"Task completion after {timealloted} timesteps: {metrics['task_completion']:.4%}")

        metriclist.append((selected_map, metrics['gained_true_utility'], metrics['total_true_utility'], metrics['task_completion']))
    print("\nSummary of results:")
    for map_id, gained_utility, total_utility, completion in metriclist:
        print(f"Map {map_id}: Gained Utility = {gained_utility:.4f}, Total Utility = {total_utility:.4f}, Task Completion = {completion:.4%}")
