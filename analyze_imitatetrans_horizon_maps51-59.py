"""
Analyzes sweep_imitatetrans_horizon_multimap.py output (maps 51-59, execution
horizons 10/20/30/40, wallclock=100s, ENFORCE_MIN_STEP_TIME=1, single run per
map/horizon, checkpoint=Imitate_best.pth) under RESULTS_ROOT (default:
results_imitate_horizon_maps51-59). Same methodology as
analyze_diffusion_horizon_maps51-59.py.

For each (map, horizon) run, computes:
  - Occupied Tr(P) drop: initial minus final occupied-region variance
    (ground-truth mask, value > 0.5), via the coarse-grid Kalman replay used
    throughout this session.
  - Final Occupied RMSE: taken directly from the run's own
    map_X_rmse_over_time.csv (last row, occupied_rmse column).
  - Occupied Tr(P)-AUC: wall-clock-time-averaged occupied variance level
    (trapezoidal over real seconds).
...
table matching the diffusion table's format.
"""
import csv
import os
import re
...
RESULTS_ROOT = Path(os.environ.get("RESULTS_ROOT", str(SCRIPT_DIR / "results_imitate_horizon_maps51-59")))
SUMMARY_CSV = RESULTS_ROOT / "summaries" / "imitate_horizon_multimap_summary.csv"
...
def main():
    ...
    out_csv = SCRIPT_DIR / "results_imitate_horizon_maps51-59_analysis.csv"
    ...
    per_run_csv = SCRIPT_DIR / "results_imitate_horizon_maps51-59_per_run.csv"
