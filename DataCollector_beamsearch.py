...
final_dataset_path = script_dir / "CMAES_beamsearch_dataset.pt"
    ...
    for selected_map in range(initial_map, initial_map + mapcount):
        ...
        for rank in range(RANKLIM):
            ...
            if mpi_rank == 0:
                ...
                ranked_candidates = select_next_beam(candidates, beam_width=len(candidates))
                for candidate in ranked_candidates[:TOP_K]:
                    record_candidate(
                        map_dataset,
                        candidate,
                        beam_contexts[candidate["parent_beam_index"]]["record_state"],
                        selected_map,
                        rank,
                        X.shape,
                        candidate["baseline_rmse"],
                    )
                ...
                del candidates, ranked_candidates, next_candidates
            else:
                beam = None
