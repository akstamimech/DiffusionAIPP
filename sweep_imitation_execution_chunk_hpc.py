...
def main():
    ...
    for map_id, execution_chunk, repeat in _task_subset(_chunk_grid()):
        ...
        variant = f"imitation_chunk{execution_chunk}"
        print(
            f"[imitation execution_chunk sweep] map={map_id} "
            f"execution_chunk={execution_chunk} repeat={repeat}",
            flush=True,
        )
        row = _run_one(
            PlannerConfig(
        ...
        row["planner"] = "imitation"
        row["planner_variant"] = variant
        row["sweep"] = "execution_chunk"
        row["execution_chunk"] = execution_chunk
        row["chunk_repeat"] = repeat
        rows.append(row)
        _write_summary("imitation_execution_chunk", rows, task_id=task_id)
        if row["return_code"] != 0 and os.environ.get("STOP_ON_FAILURE", "0") == "1":
            raise SystemExit(row["return_code"])

    summary_path = _write_summary("imitation_execution_chunk", rows, task_id=task_id)
    print(f"[imitation execution_chunk sweep] wrote {summary_path}", flush=True)
