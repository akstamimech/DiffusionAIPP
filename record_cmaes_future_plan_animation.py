"""
Top-down animation of CMAES_classic_singlemap.py over a GRF map, showing the
...
that future path is exactly what planned_path_history already tracks
(spline_path[spline_idx:]) in the original script, just captured every frame
instead of only at replan events.

Background is the GP posterior mean (not variance) field, matching the
established convention for these presentation animations.
"""
import os
...
OUT_MP4 = SCRIPT_DIR / f"cmaes_future_plan_map{sim.selected_map}_{sim.MAPTYPE}.mp4"
...
def main():
    ...
    mu_frames = [mu.copy()]
    plan_frames = [[]]  # future plan (list of (x,y)) at each captured step

    control_waypoints = []
    spline_path = []
    spline_idx = 0

    print("Running CMA-ES flight over GRF map (real optimization each replan)...")
