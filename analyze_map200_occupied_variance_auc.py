...
truth mask value > 0.5) for the three planners already run on the synthetic
four-corner map 200 (CMA-ES, Diffusion, ImitateTrans; run-tag
corners_start5050, 300s wall clock each, all starting at (50,50)). Same
coarse-grid Kalman replay methodology used throughout this session
...
PLANNERS = [
    ("cmaes", "CMA-ES expert", VIZ_DIR / f"classic_map_grf_{MAP_ID}_viz_corners_start5050"),
    ("diffusion", "Diffusion planner", VIZ_DIR / f"diffusion_map_grf_{MAP_ID}_viz_corners_start5050"),
    ("imitation", "ImitateTrans", VIZ_DIR / f"imitate_trans_map_grf_{MAP_ID}_viz_corners_start5050"),
]
...
def main():
    ...
    header = f"{'planner':>12} {'v0':>10} {'v_final':>10} {'wall_time_s':>13} {'occ_var_AUC':>13}"
    print(header)
    print("-" * len(header))
    for r in results:
        print(
            f"{r['label']:>12} {r['v0']:>10.3f} {r['v_final']:>10.3f} "
            f"{r['final_wall_time_s']:>13.1f} {r['occupied_variance_auc']:>13.3f}"
        )

    out_csv = SCRIPT_DIR / "results_map200_occupied_variance_auc.csv"
    import csv
