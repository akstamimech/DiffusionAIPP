...
"""
This script runs evaluations for several maps (indexed by number) and stores said results in CSV files.
The diffusion model checkpoint used for planning can be specified by changing the `diffusion_path` variable. 
The script is designed to be run in batch mode, allowing for easy evaluation of multiple maps in one go.
...
output_root = SCRIPT_DIR / "diffusion_batch_metrics"

diffusion_path = DIFFUSION_DIR / "checkpoints" / "sparse_trans_waypoints_epoch_2000_K1_currentbest.pth"


def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate diffusion planner over an inclusive range of map ids, "
            "running each map multiple times with different diffusion seeds."
        )
    )
    ...
    parser.add_argument("--summary-name", default="diffusion_batch_rmse_summary.csv")
...
def run_map(
    ...
    output_dir = batch_output_root / f"diffusion_map_{selected_map}_run_{run_index:02d}"
    output_dir.mkdir(parents=True, exist_ok=True)

    _, _, mean, cov, xs, ys, X, Y, xmin, xmax, ymin, ymax, gp_step = gp_context
    ...
    print(
        ...
        f"diffusion_seed={diffusion_seed}"
    )
    ...
    for ts in range(0, timealloted):
        if ts <= 1:
            grad_x, grad_y, waypoint_reached = waypoint(cx, cy, goal_x=80.0, goal_y=80.0, step=gp_step)
            prev_x, prev_y = cx, cy
            ...
            if spline_idx < len(spline_path):
                ...
                grad_x, grad_y, waypoint_reached = waypoint(
                    cx, cy, goal_x=goal_x, goal_y=goal_y, step=gp_step
                )
