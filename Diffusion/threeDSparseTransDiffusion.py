...
def ddpo_ddim_sample_timestep(
    ...
    # Paper Section 4.3's min_sampling_denoising_std (AID ships 0.05): floor the std of the noise
    # ACTUALLY ADDED to the sample, separately from the likelihood floor below. The
    # schedule-derived posterior variance decays toward zero at the late (least-noisy) denoising
    # steps - exactly the fine-tuned ones - so without this floor, on-policy exploration
    # collapses precisely where DPPO's gradients apply. mean_theta (the `direction` term above)
    # deliberately still uses the raw posterior variance - only the additive noise is floored,
    # mirroring DPPO's own implementation (std clipped at sampling time, mean untouched).
    sampling_variance = posterior_variance_t.clamp_min(min_sampling_std**2)
    x_prev =  mean_theta + torch.sqrt(sampling_variance)*noise
    mu_prior = x
    ...
    # step. Floored separately from (and higher than) min_sampling_std, matching AID's
    # min_logprob_denoising_std=0.1 vs min_sampling_denoising_std=0.05 split - the likelihood is
    # a smoothed model of the sampling distribution, not an exact match, by design.
