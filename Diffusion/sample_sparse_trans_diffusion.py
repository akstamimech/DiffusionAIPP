...
from SparseTransDiffusion import pytorch_cubic_spline

DEFAULT_CHECKPOINT = (
    Path(__file__).resolve().parent
    / "checkpoints"
    / "sparse_trans_waypoints_epoch_1400.pth"
)
CONDITION_INDEX = 22
TRUTH_INDEX = 22
SEED = None
PLOT_DIR = Path(__file__).resolve().parent / "plots"
OUTPUT_PATH = PLOT_DIR / "sparse_trans_sample.png"
NUM_STEPS = None
CLIP_X0 = False
...
def sample_sparse(model, condition_index=0, seed=None, num_steps=None, clip_x0=False):
    ...
    # SparseTransDiffusion.conditions is already normalized the same way as waypoints.
    current_position = diffusion.conditions[condition_index:condition_index + 1].to(device)
    # The same condition index selects the requested/ground-truth initial heading mode.
...
def plot_sample(sample, truth_index, condition_index, output_path=None):
    ...
    heading_velocity = diffusion.initial_heading_velocities[condition_index].detach().cpu()
    heading_velocity_world = heading_velocity * (diffusion.SCALE_FACTOR / 2.0)
    ...
    plt.arrow(
        ...
        heading_velocity_world[0].item(),
        heading_velocity_world[1].item(),
        color="white",
        width=0.35,
    ...
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title(f"SparseTransDiffusion DDIM sample vs ground truth ({truth_index})")
    plt.legend()
...
def main():
    ...
    print(
        "Using initial_heading_velocity from condition index "
        f"{args.condition_index}: "
