...
# Direct waypoint imitation copy: K=1 dataset, unweighted control-waypoint MSE.

"""
Direct transformer imitation baseline.

This file trains a behavioral cloning model that predicts sparse control waypoints
...
- current-position marker map
- current position coordinates
- initial heading velocity

Target:
- normalized control waypoints shaped [B, 2, 8]
"""
...
data_dict = torch.load(
    SCRIPT_DIR / "CMAES_currentstack_beamsearch_dataset_001_099_K1.pt"
)
...
def make_position_marker_maps(positions, grid_size = 51, marker_radius = 2):
    ...
    marker[torch.arange(B, device=positions.device), 0, y_idx, x_idx] = 1.0
    return marker
...
weights = torch.ones(control_waypoints.shape[0], dtype=torch.float32)
...
if "initial_heading_velocity" in data_dict:
    ...
    raw_initial_heading_velocity = dense_trajectories[:, :, 1] - raw_current_positions
...
class MeanVarMarkerCNN(nn.Module):
    def __init__(
        ...
        self.act2 = nn.SiLU()   
        self.pool2 = nn.MaxPool2d(kernel_size=3) # (B, 128, 25, 25) -> (B, 128, 8, 8)
        self.token_proj = nn.Linear(hidden_dim * 2, token_dim) #(B, 64, 128) -> (B, 64, 128)
    ...
    def forward(self, x, current_position=None):
        ...
        map_tokens = torch.flatten(x, start_dim=2).transpose(1, 2)  # [B, Height*Width, channels] [B, 64, 128]
...
class WaypointPredictor(nn.Module):
    ...
    def forward(self, meanvarmarker_map, current_position=None, initial_heading_velocity=None):
        ...
        WP_pred = self.output(wp_tokens) # [B, 8, 2]
        return WP_pred.transpose(1, 2) # [B, 2, 8]
...
def sample_plot_traj(output_path=None):
    ...
    plt.legend()
    if output_path is None:
        output_path = PLOT_DIR / "imitate_trans_sample_plot.png"
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
