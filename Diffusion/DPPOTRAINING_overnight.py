"""
DPPOTRAINING_overnight.py

Overnight/production variant of DPPOTRAINING.py - same DPPO algorithm, same hyperparameters the
user already validated in a 10-iteration trial (ENV_HORIZON=30, NUM_EPISODES_PER_ITERATION=16,
MINIBATCH_SIZE=512, SIGMA_PROB_MIN=0.3, lower actor LR), plus a handful of changes specifically
for running unattended for many hours with nobody watching the log:

    - NUM_DPPO_ITERATIONS raised well beyond what one night can plausibly finish. This is
      deliberate, not a mistake: per-iteration checkpointing means there's no cost to setting
      this too high - the run just gets stopped by the SLURM wall-clock limit whenever that
      happens, with the latest and best checkpoints already saved. Trying to predict the exact
      iteration count that fits in one night isn't worth the risk of guessing too low and
      leaving compute on the table.
    - Checkpoint pruning + a dedicated "best" checkpoint (Step 10 below), so hundreds of
      iterations of per-iteration checkpoints don't silently fill scratch storage, and so the
      single best iteration is trivially reloadable afterward instead of requiring a manual scan
      through the log to figure out which iteration number was best.
    - A real bug fix: update_policy's printed `policy_loss` was only ever the LAST minibatch
      processed that epoch (never averaged, unlike clip_fraction/approx_kl which already were
      correctly averaged) - now it's a proper per-epoch mean like the other two metrics.
...
Implemented so far:
    ...
    Step 8 (visualization / checkpointing): after every iteration, if VISUALIZE is True, saves a
        ...
        `rl_correction_images_overnight/` (plot_iteration_trajectory), so training instability can
        be diagnosed visually instead of only from aggregate RMSE/reward numbers. Always saves the
        current model/value_fn weights to `dppo_checkpoints_overnight/` (_save_checkpoint), so a
        specific iteration's policy can be inspected or reloaded later instead of being lost once
        training regresses past it.
    ...
    Step 9 (stability guards): collect_pooled_episodes standardizes the pooled row_advantage
        ...
        exceeds TARGET_KL (paper Appendix B's KL-divergence termination criterion). SIGMA_PROB_MIN
        (the variance floor used when evaluating the Gaussian likelihood for the PPO ratio) is
        raised from the paper's minimum of 0.1 to 0.3, cutting the likelihood's 1/sigma^2
        sensitivity to small prediction shifts by ~9x - this was the main lever that brought
        approx_kl down to comfortably under TARGET_KL in the user's 10-iteration trial.

    Step 10 (overnight-run hardening, this file only): NUM_DPPO_ITERATIONS set high enough that
        SLURM's wall-clock limit, not this loop, is what ends the run. Every iteration's
        checkpoint is still saved, but only the last CHECKPOINT_KEEP_LAST are kept on disk
        (_prune_old_checkpoints) so an unattended multi-hour run can't silently fill scratch
        storage. A separate dedicated `dppo_best.pth` (_save_best_checkpoint) is overwritten
        whenever mean_global_rmse improves on the best seen so far, so the single best iteration
        of the whole run is trivially reloadable afterward without scanning the log - this run
        does NOT rely on remembering which numbered checkpoint was best, since pruning may have
        already deleted it. update_policy's printed policy_loss is now a proper per-epoch mean
        (it previously reported only the last minibatch processed that epoch, unlike
        clip_fraction/approx_kl which were already correctly averaged).

HOW TO RUN:
    Run as a module from the `scripts` directory (not `python DPPOTRAINING_overnight.py` from
    inside Diffusion/) so that both the `Diffusion` package and its sibling modules at the
    scripts root (gaussianprocesstraining, evalmetrics, CMAES_classic_singlemap,
    Diffusionplanner_singlemap) are importable:
        python -m Diffusion.DPPOTRAINING_overnight
    ...
    updated weights each iteration). NUM_EPISODES_PER_ITERATION=16 below assumes 16 MPI ranks
    (--ntasks=16) for even load balancing - not this file's concern, but worth matching in
    whatever SLURM script launches it:
        mpiexec -n 16 python -m Diffusion.DPPOTRAINING_overnight
        srun --mpi=pmix python -m Diffusion.DPPOTRAINING_overnight
"""
...
# loop count in collect_rollouts, diffusion.T is the INNER per-replan denoising-step count in
# ddpo_ddim_sample_timestep. Just don't assume "step 30" in a log line means the same thing in
# both contexts.
ENV_HORIZON = 30
...
# the same RMSE improvement is worth strictly less the later it happens. Rebalanced from 0.01 to
# 0.003 when ENV_HORIZON went 10->30, to keep the max per-episode penalty (~0.09) roughly
# unchanged rather than tripling it (0.003 * 29 =~ 0.087).
...
# variance used for actual sampling noise, which stays at its raw schedule-derived value - see
# ddpo_ddim_sample_timestep's likelihood_variance). Paper's own minimum is 0.1; raised to 0.3
# here because approx_kl was blowing well past TARGET_KL within 1-2 epochs at 0.1, even after
# advantage normalization and gradient clipping - since the likelihood divides by this value,
# raising it from 0.1 to 0.3 cuts that sensitivity by (0.3/0.1)^2 = 9x. This was the main lever
# that brought approx_kl comfortably under TARGET_KL in the user's 10-iteration trial.
SIGMA_PROB_MIN = 0.3
...
VISUALIZE = True
# Distinct directory names from DPPOTRAINING.py's (not "rl_correction_images"/"dppo_checkpoints")
# so a real overnight run's outputs never collide with a local test run of the original file.
IMAGE_DIR = SCRIPT_DIR / "rl_correction_images_overnight"
...
CHECKPOINT_DIR = SCRIPT_DIR / "dppo_checkpoints_overnight"
...
def compute_advantages(buffer, value_fn, env, denoising_gamma=0.99, env_gamma=ENV_GAMMA, gae_lambda=GAE_LAMBDA):
    ...
    with torch.no_grad():
        ...
        # is in use) - buffer.reward was built with torch.tensor(..., dtype=torch.float32) with
        # no device argument, so it defaults to CPU, and torch.zeros(1) below defaults to CPU
        # too. Both need to match state_values' device explicitly or this crashes the moment a
        # GPU is actually in use (it silently never came up before since every prior run was
        # CPU-only, where everything already agreed by default).
...
def update_policy(buffer, discounted_advantage, env, policy_optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        # Was previously just the last minibatch processed this epoch (never averaged, unlike
        # clip_fraction/approx_kl above) - that made policy_loss bounce between epochs based on
        # which random ~minibatch_size rows happened to land last, not genuine epoch progress.
...
def _save_best_checkpoint(model, value_fn, iteration, global_rmse):
    """Overwrites a single dedicated dppo_best.pth whenever mean_global_rmse improves on the
    best seen so far this run - so the single best iteration of an unattended overnight run is
    trivially reloadable afterward without scanning the log, and without depending on whether
...
def train(env, num_iterations=NUM_DPPO_ITERATIONS, num_episodes=NUM_EPISODES_PER_ITERATION):
    ...
    Also tracks the best mean_global_rmse seen so far (rank 0 only) and saves it to a dedicated
    dppo_best.pth whenever it improves, on top of the regular per-iteration checkpoint (which
    only keeps the last CHECKPOINT_KEEP_LAST on disk) - see Step 10 in the module docstring.
    """
    ...
    if detect_mpi_launch_issue(os.environ, mpi_size):
        raise RuntimeError(
            ...
            "such as `srun --mpi=pmix python -m Diffusion.DPPOTRAINING_overnight` or "
            "`mpiexec -n $SLURM_NTASKS python -m Diffusion.DPPOTRAINING_overnight`."
        )
    if mpi_rank == 0:
    ...
    for iteration in range(num_iterations):
        ...
        if mpi_rank == 0:
            ...
            if mean_global_rmse < best_global_rmse:
                best_global_rmse = mean_global_rmse
                _save_best_checkpoint(diffusion.model, value_fn, iteration, mean_global_rmse)
                print(f"    new best global_rmse={mean_global_rmse:.4f} at iteration {iteration} - saved dppo_best.pth")
