...
def main():
    ...
    parser.add_argument("--sigma-prob-min", type=float, default=0.1)
    args = parser.parse_args()


    print(f"Loading behavioural cloning checkpoint: {args.checkpoint}")
    ...
    behavioural_cloning_model = load_model(args.checkpoint, diffusion.device)
    sample = sample_sparse(
        behavioural_cloning_model,
        condition_index=args.condition_index,
        seed=args.seed,
        num_steps=args.num_steps,
        clip_x0=args.clip_x0,
        sigma_prob_min=args.sigma_prob_min
    )
